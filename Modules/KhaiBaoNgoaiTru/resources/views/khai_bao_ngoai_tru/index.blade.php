...
@section('title', 'Quản lý khai báo ngoại trú')
...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h2 class="mb-1 fw-bold">
            <i class="fas fa-building me-2"></i>Quản lý Khai báo Ngoại trú
        </h2>
        <p class="mb-0 opacity-75 small">Danh sách sinh viên đã khai báo nơi ở</p>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('khai_bao_ngoai_tru.tao_ky') }}" class="btn btn-warning rounded-pill px-4 fw-medium">
            <i class="fas fa-calendar-plus me-1"></i> Tạo kỳ khai báo
        </a>
        <a href="{{ route('khai_bao_ngoai_tru.create') }}" class="btn btn-light rounded-pill px-4 fw-medium">
            <i class="fas fa-plus me-1"></i> Thêm khai báo
        </a>
    </div>
</div>
...
<div class="card border-0 shadow-sm rounded-3 mb-4">
    <div class="card-body py-3">
        <form method="GET" action="{{ route('khai_bao_ngoai_tru.index') }}" class="row g-3">
            <div class="col-md-8">
            ...
            @if(request('search'))
            <div class="col-md-2">
                <a href="{{ route('khai_bao_ngoai_tru.index') }}" class="btn btn-outline-secondary w-100 rounded-pill">Xóa lọc</a>
            </div>
            @endif
        </form>
    </div>
            ...
            <tbody>
                ...
                <tr class="student-row">
                    ...
                    <td class="text-center pe-4">
                        <div class="btn-group">
                            <a href="{{ route('khai_bao_ngoai_tru.show', $item->id) }}" 
                               class="btn btn-sm btn-outline-primary rounded-start-pill px-3" title="Xem chi tiết">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ route('khai_bao_ngoai_tru.edit', $item->id) }}" 
                               class="btn btn-sm btn-outline-warning rounded-0" title="Sửa">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('khai_bao_ngoai_tru.destroy', $item->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="7" class="text-center py-5">
                        ...
                        <h6 class="text-muted">Chưa có khai báo ngoại trú nào</h6>
                    </td>
                </tr>
                @endforelse
            </tbody>
